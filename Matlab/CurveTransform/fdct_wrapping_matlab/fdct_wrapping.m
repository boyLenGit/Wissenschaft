function [C,LenTV] = fdct_wrapping(x, is_real, finest, nbscales, nbangles_coarse)
...
for j = scales,
    ...
    for quadrant = 1:nbquadrants
        ...
        for subl = 2:(nbangles_perquad-1);
            ...
            coord_left = 1/2 + floor(4*M_vert)/(wedge_endpoints(subl) - wedge_endpoints(subl-1)) * (wrapped_XX - mid_line_left)./(floor(4*M_vert)+1 - wrapped_YY);
            ...
            coord_right = 1/2 + floor(4*M_vert)/(wedge_endpoints(subl+1) - wedge_endpoints(subl)) * (wrapped_XX - mid_line_right)./(floor(4*M_vert)+1 - wrapped_YY);
        ...
        first_row = floor(4*M_vert)+2-ceil((length_corner_wedge+1)/2) + mod(length_corner_wedge+1,2)*(quadrant-2 == mod(quadrant-2,2));
        first_col = floor(4*M_horiz)+2-ceil((width_wedge+1)/2) + mod(width_wedge+1,2)*(quadrant-3 == mod(quadrant-3,2));
        for row = Y_corner
            cols = left_line(row) + mod((0:(width_wedge-1))-(left_line(row)-first_col),width_wedge);
        ...
        coord_left = 1/2 + floor(4*M_vert)/(wedge_endpoints(end) - wedge_endpoints(end-1)) * (wrapped_XX - mid_line_left)./(floor(4*M_vert) + 1 - wrapped_YY);
        ...
        wrapped_XX((wrapped_XX - 1)/floor(4*M_horiz) == (wrapped_YY - 1)/floor(4*M_vert)) = wrapped_XX((wrapped_XX - 1)/floor(4*M_horiz) == (wrapped_YY - 1)/floor(4*M_vert)) - 1;
        coord_corner = C1 + C2 * (2-((wrapped_XX - 1)/(floor(4*M_horiz)) + (wrapped_YY - 1)/(floor(4*M_vert)))) ./ ((wrapped_XX - 1)/(floor(4*M_horiz)) - (wrapped_YY - 1)/(floor(4*M_vert)));
        wl_left = fdct_wrapping_window(coord_left);
